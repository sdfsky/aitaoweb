...
function dirfile_check(&$dirfile_items) {
	foreach($dirfile_items as $key => $item) {
		$item_path = $item['path'];
		if($item['type'] == 'dir') {
			if(!dir_writeable(TIPASK_ROOT.$item_path)) {
				if(is_dir(TIPASK_ROOT.$item_path)) {
			...
			if(file_exists(TIPASK_ROOT.$item_path)) {
				if(is_writable(TIPASK_ROOT.$item_path)) {
					$dirfile_items[$key]['status'] = 1;
					$dirfile_items[$key]['current'] = '+r+w';
				} else {
					$dirfile_items[$key]['status'] = 0;
				...
				if(dir_writeable(dirname(TIPASK_ROOT.$item_path))) {
					$dirfile_items[$key]['status'] = 1;
					$dirfile_items[$key]['current'] = '+r+w';
				} else {
					$dirfile_items[$key]['status'] = -1;
	...
	foreach($env_items as $key => $item) {
		if($key == 'php') {
			...
			if(function_exists('disk_free_space')) {
				$env_items[$key]['current'] = floor(disk_free_space(TIPASK_ROOT) / (1024*1024)).'M';
			} else {
...
function show_header() {
	...
	global $step;
	$version = TIPASK_VERSION;
	$release = TIPASK_RELEASE;
	$name=SOFT_NAME;
...
function show_footer($quit = true) {
	...
	echo <<<EOT
		<div class="footer">&copy;2010 - 2014 <a href="http://www.tipask.com/" target="_blank">Tipask</a> Inc.</div>
	</div>
</div>
</body>
</html>
...
function config_edit() {
	...
	$config .= "define('TIPASK_VERSION', '".TIPASK_VERSION."');\r\n";
	$config .= "define('TIPASK_RELEASE', '".TIPASK_RELEASE."');\r\n";
	$fp = fopen(CONFIG, 'w');
	fwrite($fp, $config);
	fclose($fp);
}
...
function check_adminuser($username, $password, $email) {
	...
	@include TIPASK_ROOT.'./config.php';
	include TIPASK_ROOT.'./uc_client/client.php';
	$error = '';
