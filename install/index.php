...
require 'var.inc.php';
require TIPASK_ROOT.'./install/install.lang.php';
require TIPASK_ROOT.'./install/func.inc.php';
...
if($method == 'show_license') {
	...
	/*
	$jsonstr=topen("http://www.tipask.com/site/checkver.php",3);
	if(!empty($jsonstr)){
		$json = json_decode($jsonstr);
		if($json && $json->release>TIPASK_RELEASE){
			echo '<script type="text/javascript">if(confirm("本Tipask版本较低，是否去官方下载功能更完善的最新版本?")){window.location="http://www.tipask.com/";}</script>';
		}
	}
	*/
	...
	if($submit) {
		...
		dir_clear(TIPASK_ROOT.'./data/cache');//clear up the old data cathe
	...
	if(file_exists($lockfile)) {
		@touch(TIPASK_ROOT.'./data/install.lock');
