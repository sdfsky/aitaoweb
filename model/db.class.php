...
class dbmodel {
    ...
    function show_table_status() {
        $statuslist=array();
        $query=$this->db->query("SHOW TABLE STATUS LIKE '".DB_TABLEPRE."%'");
    ...
    function optimize_table($table) {
        $optimize=$this->db->query("OPTIMIZE TABLE ".$table);
        return intval(!empty($optimize));
    }

    function repair_table($table) {
        $repair=$this->db->query("REPAIR TABLE ".$table);
        return intval(!empty($repair));
    }
